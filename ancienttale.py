...
PEARL_QUESTIONS = [
    ...
    ("Large white/pinkish pearl?", ["Danah", "Yaqooti", "Jiwan"], "Jiwan"),
...
# Pearl coordinates on background (inside image)
PEARL_POSITIONS = [(500,100),(500,220),(500,340),(500,460)] 
PEARL_SIZE = (80,80)

# -----------------------
# Helper Functions
# -----------------------
def single_scene(image_path, next_name):
    container = st.empty()
    with container:
        show(load_img(image_path))
        if st.button("Next ▶"):
            next_scene(next_name)
            container.empty()  # clears previous scene
...
if st.session_state.scene == "dubainew1":
    single_scene(f"dubainew1{suf}.png", "dubainew2")
elif st.session_state.scene == "dubainew2":
    single_scene(f"dubainew2{suf}.png", f"welcome1_{suf}")
elif st.session_state.scene.startswith("welcome1"):
    single_scene(f"welcome1{suf}.png", f"welcome2_{suf}")
elif st.session_state.scene.startswith("welcome2"):
    single_scene(f"welcome2{suf}.png", f"kid1{suf}")
elif st.session_state.scene.startswith("kid1"):
    single_scene(f"kid1{suf}.png", f"kid2{suf}")
    ...
    single_scene(f"kid4{suf}.png", f"crew1{suf}")

# --- CREW SCENES 1..9 ---
for i in range(1,10):
    if st.session_state.scene == f"crew{i}{suf}":
        next_name = f"crew{i+1}{suf}" if i<9 else "pearlgame"
        single_scene(f"crew{i}{suf}.png", next_name)

# --- PEARL GAME ---
if st.session_state.scene == "pearlgame":
    bg = load_img(f"pearlgame{suf}1.png")
    pearl_files = [f"pearl{i+1}.png" for i in range(4)]
    pearl_imgs = [load_img(p) for p in pearl_files]

    # Compose pearls on background
    composite = bg.copy()
    for idx, p in enumerate(pearl_imgs):
        if p:
            composite.paste(p.resize(PEARL_SIZE).convert("RGBA"), PEARL_POSITIONS[idx], p.resize(PEARL_SIZE).convert("RGBA"))
    ...
    with container:
        ...
        # Clickable pearl buttons (on image)
        for idx in range(4):
            disabled = idx < st.session_state.pearls_answered
            if st.button(f"", key=f"pearl_btn_{idx}", disabled=disabled):
